...
# The hooks below patch the V1 runner (vllm.v1.worker.gpu_model_runner). vLLM 0.29 picks
# the V2 runner (vllm.v1.worker.gpu.model_runner) for this model unless told otherwise,
# and then the hooks never fire: the capture runs to the end with rows=0 and every
# sequence incomplete. No speculation happens here, so the V1 runner is the right one.
