...
Warming the estimate pass does not cure it: a build that ran `profile_cudagraph_memory` once uncounted before the counted call (tried on card 1, fresh volume) still refused at -1.2 GiB with the counted estimate at 5.3 GiB (equivalent utilization 0.6778), so on this box the cost is paid on every estimate call of a cold-compiled process, not once; and the same build measured 0.24 GiB less on a warm boot (4.07 GiB, pool 263,716 against 4.31 and 279,646), so the extra pass leaves state behind. That build is not on the branch. What the patch on the branch does is the profiling pass only (fork tip `512a9699c`, the same tree as the earlier export under a corrected message), and the open item stays open with a sharper name: the WSL2 4090's first `CTX=huge` boot on a fresh volume, and the 0.29 graph estimate on that box.
