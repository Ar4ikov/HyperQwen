...
On the default profile the fix makes the cold boot measure exactly what the warm boot measures, on both boxes: same available KV, same pool, same equivalent utilization (0.8514 on the 3090 in all four rows, 0.8522 on the 4090; that line never distinguished a failure from a pass on either box, so it is not the explanation). On `CTX=huge` the fix recovers 0.89 GiB of the cold-boot shortfall on the WSL2 4090 and the boot still fails there at `GPU_UTIL=0.90` (one box, one try), while the same cold boot passes on the native 3090 with a pool within a percent of the 4090's warm pool. So the remaining refusal is box-shaped, not a property of the profile or the fix. Its failure shape differs from B's too: a negative available figure means the profile consumed more than the budget, which points at allocator state left behind by the KVarN kernels' first JIT (104 s on the 4090) rather than the compile pass the fix addresses. The warm boot on that profile passes on both boxes. What the falsifier shows is that the counted window no longer includes compiler scratch; it does not show that no box can exhaust memory on a cold boot. Open: the WSL2 4090's first `CTX=huge` boot on a fresh volume (a lower first-boot `GPU_UTIL`, or a warmup of the KVarN kernels before profiling).
