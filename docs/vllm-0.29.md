...
| native 3090 | B default, cold, 0.28.0 (main's CI image `sha-684e927`) | not read | 4.63 GiB (4.76 needed) | none | refused at 182 s |
...
On the default profile the fix makes the cold boot measure exactly what the warm boot measures, on both boxes: same available KV, same pool, same equivalent utilization (0.8514 on the 3090 in all four rows, 0.8522 on the 4090). 0.28.0's own cold default boot is not clean either: it passes on the WSL2 4090 (the campaign's B arm, pool 66,692) and refuses on the native 3090 (4.63 GiB available against 4.76 needed, main's CI image), so 0.29.0's profiling change costs 0.21 GiB on that card (4.63 to 4.42) and widens a margin that was already negative there rather than creating the failure; the fix clears a bar 0.28.0 never cleared on it (5.37 GiB, 0.74 above 0.28.0's cold figure).
...
Warming the estimate pass does not cure it: a build that ran `profile_cudagraph_memory` once uncounted before the counted call (tried on card 1, fresh volume) still refused at -1.2 GiB with the counted estimate at 5.3 GiB (equivalent utilization 0.6778), so on this box the cost is paid on every estimate call of a cold-compiled process, not once; and the same build measured 0.24 GiB less on a warm boot (4.07 GiB, pool 263,716 against 4.31 and 279,646), so the extra pass leaves state behind. That build is not on the branch. What the patch on the branch does is the profiling pass only (fork tip `512a9699c`, the same tree as the earlier export under a corrected message), and the open item stays open with a sharper name: the 0.29 graph estimate on the WSL2 4090 under `CTX=huge`.

What works on that box is `VLLM_MEMORY_PROFILER_ESTIMATE_CUDAGRAPHS=0`. With it the cold `CTX=huge` boot on a fresh volume passes at 4.47 GiB (pool 290,265, health at 283 s, a request served; the engine still computes the estimate, 5.44 GiB, against an actual graph pool of 0.0 GiB) and the warm boot at 4.68 GiB (pool 302,654, health at 127 s, estimate 0.37 GiB, actual 0.0). The knob is part of the compile-cache key (a second `torch_compile_cache` directory appears when it flips), so a boot with it back on is cold again and refuses at -0.97 GiB as before: on this box it is a setting for the profile, not a first-boot-only step. The path is verified only where the actual graph pool reads 0.0 GiB, which on the WSL2 4090 it does on every profile by the engine's own measurement (two boots serving one request each, not a run under load). On the native 3090 the same measurement reads 0.11 GiB on the default and long profiles, 1.27 GiB on the dflash2 fast control and 1.87 GiB on the production line, so disabling the estimate there under-reserves memory the engine goes on to use, and the failure would come under load, not at boot: keep it on wherever the actual pool is not zero. Whether the WSL2 zero is the pool or the measurement is not settled. The two traps sit together: a knob that looks first-boot-only and is a permanent property of the profile, and a reservation that looks spurious and is real on the other card. On the default profile the equivalent-utilization line is identical across failing and passing boots and is not the explanation; on `CTX=huge` it separates them, because the term it reports is the estimate, which is the thing that moves.
