...
Warming the estimate pass does not cure it: a build that ran `profile_cudagraph_memory` once uncounted before the counted call (tried on card 1, fresh volume) still refused at -1.2 GiB with the counted estimate at 5.3 GiB (equivalent utilization 0.6778), so on this box the cost is paid on every estimate call of a cold-compiled process, not once; and the same build measured 0.24 GiB less on a warm boot (4.07 GiB, pool 263,716 against 4.31 and 279,646), so the extra pass leaves state behind. That build is not on the branch. What the patch on the branch does is the profiling pass only (fork tip `512a9699c`, the same tree as the earlier export under a corrected message), and the open item it left is closed by the second fix below.

`VLLM_MEMORY_PROFILER_ESTIMATE_CUDAGRAPHS=0` boots that profile on that box (cold on a fresh volume 4.47 GiB, pool 290,265, health at 283 s, a request served; warm 4.68 GiB, pool 302,654, health at 127 s), and the knob is part of the compile-cache key (a second `torch_compile_cache` directory appears when it flips, so a boot with it back on is cold again). It is not the fix, because the pool it stops reserving is real. Read at the same two points the driver is read, by the allocator (`torch.cuda.memory_reserved`, a bind-mounted `gpu/model_runner.py`, card 1, warm boots): the real capture after KV allocation reserves 0.232 GiB on `CTX=huge` and 0.092 GiB on the default profile, while the driver's free-memory figure reads 0.000 GiB before and after it, so the logged "actual 0.0" on this box is the driver reading pinned at zero once the KV cache fills the budget, not the pool. On the native 3090 the driver delta reads 0.24 GiB on `CTX=huge` and 0.11 on the default (threadchip; his allocator reading is pending, so the native driver-to-allocator gap is not measured yet, and the two figures above are two instruments on two boxes). And the cold `CTX=huge` estimate on card 1, read the same way: the throwaway capture reserves 0.232 GiB, the same as warm, while the driver's free-memory figure falls from 5.436 GiB to 0.000 across it; that 5.44 is the "estimate" the profiler subtracted from the KV budget. The 0.29 estimator is built on the driver's free-memory delta, and under WSL2's paravirtual driver that reading collapses during the first compile of the KVarN kernels. The estimate's own capture also over-reserves about 1 GiB on the default profile on both platforms (1.03 GiB reserved against 0.09 real on card 1; 1.14 estimated against 0.11 actual on the native 3090): that is the estimate pass's shape, not WSL2, and it is unchanged by anything here.

So the branch carries a second fix, `cudagraph-memory-from-allocator.patch`: `capture_model` returns the allocator's reserved delta and logs the driver delta beside it, and the FULL-graph samples the estimate extrapolates from are read the same way, so the estimate never mixes an allocator total with driver samples. What that reading knowingly leaves unreserved is the driver-side residue of a capture (graph executables, kernel module loads, per-kernel local memory, non-torch workspaces); on the native 3090 it is the gap between the driver delta and the allocator delta, pending measurement; on the WSL2 box the driver cannot measure it at all. Verified on card 1 with the estimate on: the cold `CTX=huge` boot on a fresh volume passes (4.19 GiB, pool 270,796, health at 288 s, a request served; the estimate reads 0.28 GiB for a 0.23 GiB pool while the driver's delta across the same capture still reads 5.44), the warm boot reads 4.41 GiB (pool 284,955), and the cold default boot 5.77 GiB (pool 79,414, 0.09 above the profiling fix alone). The 0.22 GiB between cold and warm on `CTX=huge` is the profile-side term above, which remains. The default profile's estimate is still about a GiB above its real pool (1.05 against 0.09), which answers which half of the estimate over-reserves: the throwaway capture itself reserves 1.03 GiB where the real capture reserves 0.09, so the extrapolation from FULL-graph samples is not the term. The logged "actual" now reads the pool (0.23 and 0.09) instead of 0.0. On the default profile the equivalent-utilization line is identical across failing and passing boots and is not the explanation; on `CTX=huge` it separates them, because the term it reports is the estimate, which is the thing that moves.
