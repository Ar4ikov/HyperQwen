...
| turboquant-skip-layers-by-name | fix | `--kv-cache-dtype-skip-layers sliding_window` (an attention type name, which the option documents) with a `turboquant_*` cache: the boundary-layer merge sorted the list with `key=int` and died on the name; the drafter's sliding-window layers need it to stay bf16 (TurboQuant has no window mask) | none yet | 0.29.0 | upstream PR |
| turboquant-spec-as-decode | fix | the TurboQuant backend (`--kv-cache-dtype turboquant_*`) treats the speculative verify block (1+k queries per request) as a decode: the builder expands it into one decode row per token (seq_len = context so far + 1, the request's block table repeated) in persistent buffers and the decode kernel attends all rows in one launch, exact and CUDA-graph safe; stock 0.29.0 sends it through the prefill path's per-request Python loop on CPU metadata, which the captured uniform verify graphs then replay with stale shapes | none yet | 0.29.0 | upstream PR |
